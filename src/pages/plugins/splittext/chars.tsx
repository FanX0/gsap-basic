...
const Chars = () => {
  ...
  const read = () => setCount(stRef.current?.chars?.length ?? 0);

  return (
    <DocumentationLayout>
      <section className="space-y-6 pt-10">
        <p ref={textRef} className="text-lg">Split this into characters.</p>
        {!hasSplitText && <div className="text-sm text-red-600">SplitText is not available.</div>}
        <div className="text-sm">chars.length: {count}</div>
