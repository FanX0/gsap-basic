...
const Masks = () => {
  ...
  useEffect(() => {
    ...
    stRef.current = new S(textRef.current, { type: "lines", mask: true });
    return () => {
  ...
  const read = () => setCount(stRef.current?.masks?.length ?? 0);

  return (
    <DocumentationLayout>
      <section className="space-y-6 pt-10">
        <p ref={textRef} className="text-lg w-64">Masked lines.</p>
        {!hasSplitText && <div className="text-sm text-red-600">SplitText is not available.</div>}
        <div className="text-sm">masks.length: {count}</div>
