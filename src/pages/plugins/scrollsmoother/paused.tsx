...
const PausedMethod = () => {
  ...
  type Smoother = { paused?: (value?: boolean) => boolean; kill?: () => void };
  ...
  const read = () => setPaused(Boolean(smootherRef.current?.paused?.()));
  const enable = () => smootherRef.current?.paused?.(true);
  const disable = () => smootherRef.current?.paused?.(false);
  ...
  return (
    <DocumentationLayout>
      <section className="space-y-6 pt-10">
        ...
        <div className="text-sm">paused(): {String(paused)}</div>
        <div className="flex gap-2">
          <button className="px-3 py-1 border rounded" onClick={read}>read</button>
          <button className="px-3 py-1 border rounded" onClick={enable}>paused(true)</button>
          <button className="px-3 py-1 border rounded" onClick={disable}>paused(false)</button>
        </div>
      </section>
    </DocumentationLayout>
  );
