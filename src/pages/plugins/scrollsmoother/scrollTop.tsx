...
const ScrollTopMethod = () => {
  ...
  type Smoother = { scrollTop?: (value?: number) => number; kill?: () => void };
  ...
  const read = () => setTop(Number(smootherRef.current?.scrollTop?.() ?? 0));
  const add200 = () => smootherRef.current?.scrollTop?.(top + 200);
  ...
  return (
    <DocumentationLayout>
      <section className="space-y-6 pt-10">
        ...
        <div className="text-sm">scrollTop(): {top.toFixed(0)}</div>
        <div className="flex gap-2">
          <button className="px-3 py-1 border rounded" onClick={read}>read</button>
          <button className="px-3 py-1 border rounded" onClick={add200}>scrollTop(+200)</button>
        </div>
      </section>
    </DocumentationLayout>
  );
...
export default ScrollTopMethod;
