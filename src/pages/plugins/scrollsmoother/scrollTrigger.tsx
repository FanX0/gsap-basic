...
const STPropScrollTrigger = () => {
  const wrapperRef = useRef<HTMLDivElement | null>(null);
  const contentRef = useRef<HTMLDivElement | null>(null);
  type Smoother = {
    scrollTrigger?: { progress?: number } | null;
    kill?: () => void;
  };
  ...
  const read = () => setStProgress(Number(smootherRef.current?.scrollTrigger?.progress ?? 0));
  ...
  return (
    <DocumentationLayout>
      <section className="space-y-6 pt-10">
        ...
        <div className="text-sm">scrollTrigger.progress: {stProgress.toFixed(2)}</div>
...
export default STPropScrollTrigger;
