...
type InertiaStatic = { untrack?: (target: Element | object, props?: string | string[]) => void };

const InertiaUntrack = () => {
  const boxRef = useRef<HTMLDivElement | null>(null);
  const getI = () => (globalThis as { InertiaPlugin?: InertiaStatic }).InertiaPlugin;
  const hasPlugin = Boolean(getI());

  const run = () => getI()?.untrack?.(boxRef.current!, "x");
