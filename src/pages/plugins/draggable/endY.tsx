...
type DraggableInstance = { endY?: number; kill?: () => void };
...
const EndYProp = () => {
  ...
  useEffect(() => {
    ...
    instRef.current = D.create?.(boxRef.current, { type: "y", bounds: { minY: -100, maxY: 100 } })?.[0] ?? null;
    return () => instRef.current?.kill?.();
  }, []);

  const read = () => setVal(Number(instRef.current?.endY ?? 0));
  ...
  return (
    <DocumentationLayout>
      <section className="space-y-6 pt-10">
        ...
        <div className="text-sm">endY: {val.toFixed(0)}</div>
