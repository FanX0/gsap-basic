import { useEffect, useRef, useState } from "react";
import DocumentationLayout from "@/components/layouts/DocumentationLayout";

type DraggableInstance = { enabled?: () => boolean; kill?: () => void; disable?: () => void; enable?: () => void };
type DraggableStatic = { create?: (target: Element | Element[] | string, vars?: unknown) => DraggableInstance[] };

const DGEnabled = () => {
  const boxRef = useRef<HTMLDivElement | null>(null);
  const instRef = useRef<DraggableInstance | null>(null);
  const [val, setVal] = useState(false);
  const getD = () => (globalThis as { Draggable?: DraggableStatic }).Draggable;
  const hasDraggable = Boolean(getD());

  useEffect(() => {
    const D = getD();
    if (!D || !boxRef.current) return;
    const inst = D.create?.(boxRef.current, { type: "x,y" })?.[0] ?? null;
    instRef.current = inst;
    inst?.disable?.();
    return () => instRef.current?.kill?.();
  }, []);

  const read = () => setVal(Boolean(instRef.current?.enabled?.()));
  const toggle = () => {
    const e = instRef.current?.enabled?.();
    if (e) instRef.current?.disable?.(); else instRef.current?.enable?.();
  };

  return (
    <DocumentationLayout>
      <section className="space-y-6 pt-10">
        <div ref={boxRef} className="size-10 bg-indigo-600" />
        {!hasDraggable && <div className="text-sm text-red-600">Draggable is not available.</div>}
        <div className="text-sm">enabled(): {String(val)}</div>
        <div className="flex gap-2">
          <button className="px-3 py-1 border rounded" onClick={read}>read</button>
          <button className="px-3 py-1 border rounded" onClick={toggle}>toggle</button>
        </div>
      </section>
    </DocumentationLayout>
  );
};

export default DGEnabled;
