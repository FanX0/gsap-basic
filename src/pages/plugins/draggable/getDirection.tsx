...
const DGGetDirection = () => {
  ...
  useEffect(() => {
    ...
    instRef.current = D.create?.(boxRef.current, { type: "x" })?.[0] ?? null;
    return () => instRef.current?.kill?.();
  }, []);

  const read = () => setVal(Number(instRef.current?.getDirection?.() ?? 0));
  ...
  return (
    <DocumentationLayout>
      <section className="space-y-6 pt-10">
        ...
        <div className="text-sm">getDirection(): {val}</div>
