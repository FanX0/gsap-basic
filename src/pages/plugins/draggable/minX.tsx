...
type DraggableInstance = { minX?: number; kill?: () => void };
...
const MinXProp = () => {
  ...
  useEffect(() => {
    ...
    instRef.current = D.create?.(boxRef.current, { type: "x", bounds: { minX: -200 } })?.[0] ?? null;
    return () => instRef.current?.kill?.();
  }, []);

  const read = () => setVal(Number(instRef.current?.minX ?? 0));
  ...
  return (
    <DocumentationLayout>
      <section className="space-y-6 pt-10">
        ...
        <div className="text-sm">minX: {val.toFixed(0)}</div>
