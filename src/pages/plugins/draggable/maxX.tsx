...
type DraggableInstance = { maxX?: number; kill?: () => void };
...
const MaxXProp = () => {
  ...
  useEffect(() => {
    ...
    instRef.current = D.create?.(boxRef.current, { type: "x", bounds: { maxX: 200 } })?.[0] ?? null;
    return () => instRef.current?.kill?.();
  }, []);

  const read = () => setVal(Number(instRef.current?.maxX ?? 0));
  ...
  return (
    <DocumentationLayout>
      <section className="space-y-6 pt-10">
        ...
        <div className="text-sm">maxX: {val.toFixed(0)}</div>
