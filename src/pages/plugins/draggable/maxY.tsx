...
type DraggableInstance = { maxY?: number; kill?: () => void };
...
const MaxYProp = () => {
  ...
  useEffect(() => {
    ...
    instRef.current = D.create?.(boxRef.current, { type: "y", bounds: { maxY: 200 } })?.[0] ?? null;
    return () => instRef.current?.kill?.();
  }, []);

  const read = () => setVal(Number(instRef.current?.maxY ?? 0));
  ...
  return (
    <DocumentationLayout>
      <section className="space-y-6 pt-10">
        ...
        <div className="text-sm">maxY: {val.toFixed(0)}</div>
