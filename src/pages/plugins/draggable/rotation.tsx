...
type DraggableInstance = { rotation?: number; kill?: () => void };
...
const RotationProp = () => {
  ...
  useEffect(() => {
    ...
    instRef.current = D.create?.(boxRef.current, { type: "rotation" })?.[0] ?? null;
    return () => instRef.current?.kill?.();
  }, []);

  const read = () => setVal(Number(instRef.current?.rotation ?? 0));
  ...
  return (
    <DocumentationLayout>
      <section className="space-y-6 pt-10">
        ...
        <div className="text-sm">rotation: {val.toFixed(0)}</div>
