...
type DraggableInstance = { endX?: number; kill?: () => void };
...
const EndXProp = () => {
  ...
  useEffect(() => {
    ...
    instRef.current = D.create?.(boxRef.current, { type: "x", bounds: { minX: -100, maxX: 100 } })?.[0] ?? null;
    return () => instRef.current?.kill?.();
  }, []);

  const read = () => setVal(Number(instRef.current?.endX ?? 0));
  ...
  return (
    <DocumentationLayout>
      <section className="space-y-6 pt-10">
        ...
        <div className="text-sm">endX: {val.toFixed(0)}</div>
