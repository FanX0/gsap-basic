...
const EndRotationProp = () => {
  ...
  useEffect(() => {
    ...
    instRef.current = D.create?.(boxRef.current, { type: "rotation", bounds: { minRotation: -90, maxRotation: 90 } })?.[0] ?? null;
    return () => instRef.current?.kill?.();
  }, []);

  const read = () => setVal(Number(instRef.current?.endRotation ?? 0));

  return (
    <DocumentationLayout>
      <section className="space-y-6 pt-10">
        <div ref={boxRef} className="size-12 bg-indigo-600 rounded-full" />
        {!hasDraggable && <div className="text-sm text-red-600">Draggable is not available.</div>}
        <div className="text-sm">endRotation: {val.toFixed(0)}</div>
...
export default EndRotationProp;
